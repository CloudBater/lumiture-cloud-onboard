...
variable "lumiture_environment" {
  description = <<-EOT
    LumiTure environment to onboard against. Selects the service account to grant and
    the API endpoint. Sandbox shares the dev GCP project (and SA) but has its own API URL.
  EOT
  type        = string
  default     = "prod"

  validation {
    condition     = contains(["prod", "dev", "staging", "sandbox"], var.lumiture_environment)
    error_message = "lumiture_environment must be one of: prod, dev, staging, sandbox."
  }
}

variable "lumiture_service_account" {
  description = <<-EOT
    Override the LumiTure service account to grant. Leave null to derive from `lumiture_environment`.
    Keep in sync with frontend/organization/billing-integration.md (Guide Constants) and
    infra/secret/sa-iam-bindings.md in the LumiTure specs repo.
  EOT
  type        = string
  default     = null
}
...
variable "lumiture_api_base" {
  description = "LumiTure API base URL. Leave null to derive from `lumiture_environment`."
  type        = string
  default     = null
}
