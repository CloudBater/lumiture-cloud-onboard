# terraform-google-lumiture-onboarding (skeleton)

> **Draft skeleton** — design artifact in the specs repo. Promotes to its own
> public repo `CloudMile-Product/terraform-google-lumiture-onboarding` and
> registers on Terraform Registry once approved.
...
module "lumiture_gcp_onboarding" {
  ...
  # Defaults to prod; switch to "dev" for staging environment
  lumiture_environment = "prod"
...
| <a name="input_lumiture_api_base"></a> [lumiture\_api\_base](#input\_lumiture\_api\_base) | LumiTure API base URL. Leave null to derive from `lumiture_environment`. | `string` | `null` | no |
| <a name="input_lumiture_environment"></a> [lumiture\_environment](#input\_lumiture\_environment) | LumiTure environment to onboard against. Selects the service account to grant and<br/>the API endpoint. Sandbox shares the dev GCP project (and SA) but has its own API URL. | `string` | `"prod"` | no |
...
| <a name="input_lumiture_service_account"></a> [lumiture\_service\_account](#input\_lumiture\_service\_account) | Override the LumiTure service account to grant. Leave null to derive from `lumiture_environment`.<br/>Keep in sync with frontend/organization/billing-integration.md (Guide Constants) and<br/>infra/secret/sa-iam-bindings.md in the LumiTure specs repo. | `string` | `null` | no |
...
This skeleton lives in `changes/gcp-onboarding-automation/terraform/` for design
review. When approved:

1. Create `CloudMile-Product/terraform-google-lumiture-onboarding` (public)
2. Move this folder there (preserve git history if possible via `git mv` + push)
3. Add a `.github/workflows/release.yml` for tag-driven publishing
4. Register on Terraform Registry (https://registry.terraform.io/)
5. Update this folder with a `> MOVED — see ...` banner and remove HCL files (or symlink)

## Tested against

To be filled during Phase 2 development:
- [ ] org 12 demo (CloudMile internal — see `infra/env/prod-orgs.md`)
