locals {
  # Service account email derivation — keep in sync with
  # frontend/organization/billing-integration.md and infra/secret/sa-iam-bindings.md.
  # Sandbox shares the dev GCP project (tw-rd-app-finops-dev) → same SA as dev,
  # different API URL. See infra/env/sandbox.md.
  default_service_accounts = {
    prod    = "lumiture-client@tw-rd-app-finops-prod.iam.gserviceaccount.com"
    dev     = "lumiture-client@tw-rd-app-finops-dev.iam.gserviceaccount.com"
    staging = "lumiture-client@tw-rd-app-finops-dev.iam.gserviceaccount.com"
    sandbox = "lumiture-client@tw-rd-app-finops-dev.iam.gserviceaccount.com"
  }
  default_api_bases = {
    prod    = "https://api.lumiture.ai"
    dev     = "https://dev-api.lumiture.ai"
    staging = "https://stg-api.lumiture.ai"
    sandbox = "https://sandbox-api.lumiture.ai"
  }

  service_account = (
    var.lumiture_service_account != null
    ? var.lumiture_service_account
    : local.default_service_accounts[var.lumiture_environment]
  )

  api_base = (
    var.lumiture_api_base != null
    ? var.lumiture_api_base
    : local.default_api_bases[var.lumiture_environment]
  )
...
# Cloud Billing API (backend/platforms/gcp.md), which needs roles/billing.viewer
